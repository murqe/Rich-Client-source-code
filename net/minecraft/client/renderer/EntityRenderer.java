...
import zxc.rich.client.features.combat.Reach;
import zxc.rich.client.features.visuals.FogColor;
import zxc.rich.client.features.visuals.NoRender;
import zxc.rich.client.features.visuals.Snow;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            float reachDist = Main.instance.featureDirector.getFeature(Reach.class).isEnabled() ? Reach.reachValue.getNumberValue() : 3;
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            if (NoRender.hurtcam.getBoolValue() && Main.instance.featureDirector.getFeature(NoRender.class).isEnabled()) {
    ...
    public void orientCamera(float partialTicks) {
        ...
        if (mc.gameSettings.thirdPersonView == 1) {
            this.thirdPersonMath = MathHelper.lerp(this.thirdPersonMath, 2F, 5 * Feature.deltaTime());
        } else if (mc.gameSettings.thirdPersonView == 0) {
            this.thirdPersonMath = -1;
        } else if (mc.gameSettings.thirdPersonView == 2) {
            this.thirdPersonMath = MathHelper.lerp(this.thirdPersonMath, 1.5F, 5 * Feature.deltaTime());
        }
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            double d3 = this.thirdPersonDistancePrev + (this.thirdPersonMath - 2.5);
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (raytraceresult != null) {
                        ...
                        if (NoRender.cameraClip.getBoolValue() && Main.instance.featureDirector.getFeature(NoRender.class).isEnabled()) {
    ...
    protected void renderRainSnow(float partialTicks) {
        boolean snow = Main.instance.featureDirector.getFeature(Snow.class).isEnabled();
                    ...
                    if ((biome.canRain() || biome.getEnableSnow()) || snow) {
                        ...
                        if (k2 != l2) {
                            ...
                            if ((world.getBiomeProvider().getTemperatureAtHeight(f1, j2) >= 0.15F) && !snow) {
                                ...
                                int red = (int) (((Snow.weatherColor.getColorValue() >> 16) & 255) / 100.f);
                                int green = (int) (((Snow.weatherColor.getColorValue() >> 8) & 255) / 100.f);
                                int blue = (int) ((Snow.weatherColor.getColorValue() & 255) / 100.f);
                                bufferbuilder.pos((double) l1 - d3 + 0.5D, (double) l2, (double) k1 - d4 + 0.5D)
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        else
        {
            ...
            {
                float dist = Main.instance.featureDirector.getFeature(FogColor.class).isEnabled() ? -FogColor.distance.getNumberValue() : Config.getFogStart();
